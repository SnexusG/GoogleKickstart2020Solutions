...
ll binsearch(ll low, ll high){
    while(low < high){
        ...
        if(check(mid)){
            ...
            //cout<<mid<<" doesnt work"<<endl;
            high = mid;
        }
    }
    return low-1; //change this to high for upper bound (if element found then low and high both are correct)
}
...
int main(){
    ...
    while(t--){
        cin>>n>>d;
        get(x,n);
        cout<<"Case #"<<ogT-t<<": "<<(x[0] * binsearch(1,d/x[0]+1))<<endl;
    }
    return 0;
}