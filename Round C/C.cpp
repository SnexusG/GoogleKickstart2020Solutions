...
ll solve(vll a){
    ...
    loop0(i,3500){
        ...
        ll tempAns = 0;
        loop0(j,n){
            //index is j, offset is i, look for countS[j][prefix_sum+offset], if prefix_sum[j] == offset add 1
            if(prefix_sum[j] == offset){
                ans+=1;
                tempAns+=1;
            }
            ll avail = max((ll)0,countS[n-1][prefix_sum[j]+offset] - countS[j][prefix_sum[j]+offset]);
            ans+=avail;
            tempAns+=avail;
        }
        //cout<<offset<<" : "<<tempAns<<endl;
    }
    return ans;
}
